...
- **Tier 4 HAS BEEN ELECTED, for SC-01's raster only.** SC-01 uses a **synthetic Sigma0 raster generated to be consistent with an authored slick polygon** — the polygon is the alpha hull of a real forward drift run, and the raster is rendered from it by `scripts/synth_sar.py`. It exists because the two pixel-derived §5.2 rule terms, `contrast_db` and `edge_gradient_mean`, otherwise have nothing to measure and the score shrinks toward the base rate. It is badged **`SYNTHETIC — NOT SAR IMAGERY`** wherever it surfaces: the scene, every feature derived from it, the WS stage events and the manifest.
  - **This is scenario imagery for the demo. It is not a claim that detection was validated on real SAR.** Nothing was. Say that before a judge asks.
  - The election is scoped and does not spread: SC-02 and SC-03 have no raster, and no code path degrades into calling the generator. The rule above is unchanged — a human chose this, once, deliberately.
  - The method is stated, not tuned: dark-region geometry is the polygon itself, damping is the midpoint of the 3–10 dB range this file states for mineral oil, edge sharpness is one resolution cell, speckle is Gamma at an ENL derived from the IW GRDH product chain, and the background is sampled from the drift run's own wind field. The damping is deliberately **not** derived from `rule_contrast_pivot_db` — building the image out of the constants that score it would make P(oil) meaningless.
...
In this build there is no real SAR at all: SC-01 renders synthetic imagery (§0 Tier 4, elected, badged), and SC-02/SC-03 have no raster. Under **§0 Data Contingency Tier 2** the scenes come from Deep-SAR SOS instead, every one of them badged `GEOREFERENCE ASSIGNED — PIXELS ARE REAL SAR`, and the holdout is reconstituted against that dataset's own structure. The detection-truth claim above depends on the fallback shipping masks, which is **UNVERIFIED** — see §0.

**SC-01 · "Baltic Night Discharge"** — 110 s. Scene from **`oil/146–150`**, real Danish AIS traffic, one injected culprit track (badged `INJECTED — SIMULATED`). Full pipeline → particle rewind → culprit ignites → the run's own verdict and LR, quoted with its frame size.

> **The scene carries a synthetic Sigma0 raster in this build — `SYNTHETIC — NOT SAR IMAGERY`, §0 Tier 4, elected.** There is no real SAR on disk, so the raster is generated to be consistent with the authored slick polygon in order that the two pixel-derived §5.2 terms resolve at all. Demo imagery, not a detection result: **no accuracy figure may be quoted from it** (§16).
>
> **This line previously read `STRONG`, LR > 100. No run ever produced that.** The engine returns **`MODERATE`, LR 72.9 across a 23-vessel frame** — origin recovered to 0.215 km, t★ within one 30-minute snapshot, the authored culprit ranked first and the only vessel to clear a band. The figure was corrected to the engine rather than the weights tuned to the figure, exactly as §12's P(oil) line was before it. **The verdict band is the claim; the magnitude travels with its N.**
>
> **SC-01 is placed over open water and that is enforced, not assumed.** It was once generated over Bornholm — 80 of 116 slick vertices, the release point and ~90% of the particles ashore, with every downstream number arithmetically correct and physically meaningless. `scripts/land_guard.py` now checks slick vertices, the release point and every drift snapshot against a real land polygon and **raises**; the build stops rather than emitting it.
...
- **Never quote a detection metric from SC-01's synthetic raster.** It is §0 Tier 4 scenario imagery generated to be consistent with the slick polygon it contains, so a detector scoring well on it has measured the generator, not the world. It establishes nothing about accuracy on real SAR and carries no split, no n and no validation figure. It exists so the two pixel-derived §5.2 terms resolve; that is its entire claim.
